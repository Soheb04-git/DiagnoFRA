...
function Sidebar({
  ...
  // Non collapsible, simple container
  if (collapsible === 'none') {
    return (
      <div
        data-slot="sidebar"
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* prevent sidebar from pushing content */}
      <div data-slot="sidebar-gap" className="w-0" />

      <div
        data-slot="sidebar-container"
        className={cn(
          'fixed inset-y-0 z-10 hidden h-[100svh] w-[var(--sidebar-width)] transition-all duration-200 ease-linear md:flex',
        ...
        {...props}
      >
        <div data-slot="sidebar-inner" className="bg-sidebar flex h-full w-full flex-col rounded-lg border shadow-sm">
          {children}
        </div>
      </div>
    </div>
...
function SidebarSeparator(props: React.ComponentProps<typeof Separator>) {
  return <Separator data-slot="sidebar-separator" className={cn('mx-2 w-auto', props.className)} {...props} />
}
...
function SidebarGroupAction({ className, asChild = false, ...props }: React.ComponentProps<'button'> & { asChild?: boolean }) {
  ...
  return <Comp data-slot="sidebar-group-action" className={cn('text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0', className)} {...props} />
}
...
// Button variants for menu button (keeps API similar to original)
const sidebarMenuButtonVariants = cva(
...
function SidebarMenuAction({ className, ...props }: React.ComponentProps<'button'>) {
  return <button data-slot="sidebar-menu-action" className={cn('text-sidebar-foreground ring-sidebar-ring absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0', className)} {...props} />
}

function SidebarMenuBadge({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="sidebar-menu-badge" className={cn('text-sidebar-foreground absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium', className)} {...props} />
}

function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<'div'> & { showIcon?: boolean }) {
  // random width to mimic skeleton
...
// Submenu components
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return <ul data-slot="sidebar-menu-sub" className={cn('border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5', className)} {...props} />
}
...
function SidebarMenuSubButton({ asChild = false, size = 'md', isActive = false, className, ...props }: React.ComponentProps<'a'> & { asChild?: boolean; size?: 'sm' | 'md'; isActive?: boolean }) {
  ...
  return <Comp data-slot="sidebar-menu-sub-button" data-size={size} data-active={isActive} className={cn('flex h-7 items-center gap-2 overflow-hidden rounded-md px-2 text-sm', className)} {...props} />
}
